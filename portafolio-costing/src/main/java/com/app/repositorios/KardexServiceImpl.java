...
public class KardexServiceImpl extends AbstractRepository implements KardexApiInterfaz {
    ...
    public List<KardexReporteDto> obtenerMovimientosPorGrupo(Long empresaId, Long custodioId, String cuenta, Long instrumentoId) {
        return execute(em -> {
            ...
            WHERE k.empresa_id = ?1 
              ...
              AND k.nemo_id = ?3 
              AND k.cuenta = ?4 
            ORDER BY id, nemo_id ASC, fecha_tran ASC, 
                     CASE WHEN tipo_oper = 'INGRESO' THEN 1 ELSE 2 END ASC
            """;
            ...
            query.setParameter(1, empresaId)
                    ...
                    .setParameter(4, cuenta);

            // Corregir el warning de unchecked cast
            @SuppressWarnings("unchecked")
            List<KardexReporteDto> resultList = (List<KardexReporteDto>) query.getResultList();

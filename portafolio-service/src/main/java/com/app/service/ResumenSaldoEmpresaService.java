...
public class ResumenSaldoEmpresaService {
    ...
    /**
     * Orquesta la obtención y procesamiento del resumen de saldos.
     * Este es el método público principal que expone el servicio.
     * @return Una lista de DTOs con los datos de detalle, subtotales y total general.
     */
    public List<ResumenSaldoEmpresaDto> obtenerResumenSaldos() {
        List<ResumenSaldoEmpresaDto> resultadosRaw = ejecutarConsultaSaldos();
    ...
    /**
     * RESPONSABILIDAD 1: INTERACCIÓN CON LA BASE DE DATOS.
     * Se conecta a la BD y ejecuta la consulta nativa para obtener los saldos.
     * No contiene lógica de negocio, solo la obtención de datos.
     * @return Una lista "cruda" de saldos agrupados.
     */
    @SuppressWarnings("unchecked")
    private List<ResumenSaldoEmpresaDto> ejecutarConsultaSaldos() {
        ...
        try {
            ...
            """;
            Query query = entityManager.createNativeQuery(sql, RESUMEN_SALDO_MAPPING);
    ...
    /**
     * RESPONSABILIDAD 2: LÓGICA DE NEGOCIO Y PROCESAMIENTO.
     * Toma la lista cruda y la procesa para agregar subtotales y totales.
    ...
    private List<ResumenSaldoEmpresaDto> procesarResultadosConTotales(List<ResumenSaldoEmpresaDto> resultados) {
        ...
        final BigDecimal granTotalClp = resultados.stream().map(ResumenSaldoEmpresaDto::getSaldoClp).reduce(BigDecimal.ZERO, BigDecimal::add);
        final BigDecimal granTotalUsd = resultados.stream().map(ResumenSaldoEmpresaDto::getSaldoUsd).reduce(BigDecimal.ZERO, BigDecimal::add);
    ...
    /**
     * Calcula y asigna el porcentaje que representa el saldo CLP de un DTO sobre el total general.
     */
